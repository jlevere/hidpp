...
async function showDeviceUI(device: Device): Promise<void> {
  ...
  } catch { setText(batteryVal, "—"); }

  try {
    const d = await device.getDpi();
    setText(dpiVal, String(d));
  } catch { setText(dpiVal, "—"); }

  try {
    const ss = await device.getSmartShift();
    setText(modeVal, ss.mode);
  } catch { setText(modeVal, "—"); }

  try {
    const h = await device.getHostInfo();
    setText(hostVal, `${h.currentHost + 1}/${h.numHosts}`);
  } catch { setText(hostVal, "—"); }

  // Refresh detailed cards.
  void dpi.refresh(device).catch((e: unknown) => logError(`dpi: ${e}`));
  void smartShift.refresh(device).catch((e: unknown) => logError(`smartshift: ${e}`));
  void host.refresh(device).catch((e: unknown) => logError(`host: ${e}`));
...
async function createButtonsCard(device: Device): Promise<HTMLElement | null> {
  try {
    ...
    const NAMES: Record<number, string> = {
      50: "Left Click", 51: "Right Click",
      80: "Left Click", 81: "Right Click",
      82: "Middle Click", 83: "Back", 86: "Forward",
      195: "Gesture", 196: "Mode Shift", 215: "Thumbwheel Click",
    };
    ...
    for (const btn of buttons) {
      ...
      const r = el("div", { class: "button-row" },
        el("span", { class: "cid" }, `0x${cid.toString(16).padStart(4, "0")}`),
        el("span", { class: "name" }, name),
        b["divertable"] ? badge("divertable", "info") : el("span", {}),
      );
      content.append(r);
    }
